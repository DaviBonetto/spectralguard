...
\hypersetup{
    ...
    pdftitle={SpectralGuard: Detecting Memory Collapse Attacks in State Space Models}
}
...
\author{
    ...
    \small\href{https://github.com/DaviBonetto/spectralguard}{\githubicon\ \textbf{Code}}%
    \hspace{1.2em}%
    \href{https://huggingface.co/spaces/DaviBonetto/spectralguard-demo}{\hficon\ \textbf{Demo}}%
    \hspace{1.2em}%
    \href{https://huggingface.co/datasets/DaviBonetto/spectralguard-dataset}{\hficon\ \textbf{Dataset}}
}
\date{}
...
\newcommand{\githubicon}{\raisebox{-0.3ex}{\includegraphics[height=1.5em]{Logo Github.png}}}
\newcommand{\hficon}{\raisebox{-0.3ex}{\includegraphics[height=1.5em]{logo hugging face.png}}}
...
GSM8K~\cite{cobbe2021training} grade-school math word problems requiring multi-step chain-of-thought.
...
While the original HiSPA framework~\cite{lemercier2026hispa} demonstrated notable accuracy drops using unguided discrete token perturbations, our gradient-guided adaptation yields a massive $52.5$ percentage point performance degradation (compressing $H_{\text{eff}}$ to mere dozens of tokens). This demonstrates that directly optimizing for spectral collapse via gradient-based prompts is a highly effective attack vector, providing the ideal stress-test environment to evaluate defensive mechanisms.
...
\subsection{Experiment 4: Causal Mechanism Validation}
...
\vspace{2pt}
\begin{minipage}{\columnwidth}
{\footnotesize Values obtained with both protocols ($N=100$, thresholds $\{0.99,0.95,0.90,0.70,0.50\}$).}
\end{minipage}
\end{table}
...
\subsection{Experiment 5: Scaling and Robustness}
\label{sec:exp5}
...
\paragraph{5A: Detection Scaling (130M $\to$ 1.4B $\to$ 2.8B).}
...
\begin{table}[t]
\centering
\caption{SpectralGuard: Detection Across Model Scales (Experiment 5A)}
\label{tab:scaling}
\small
\resizebox{\columnwidth}{!}{%
...
\midrule
Mamba-130M & 130M & 0.619 & 0.672 & 0.9086 & 0.9090 & $+0.0004$ \\
...
\paragraph{5B: Multi-Seed Robustness (Mamba-130M).}
...
\paragraph{5C: Power Method Accuracy Validation.}
...
\paragraph{5D: Cross-Model Attack Transfer (130M $\to$ 2.8B).}
...
Figure~\ref{fig:phase4} summarizes Experiments 5A--5D visually.
...
\subsection{Experiment 6: Hybrid Architecture Validation (Zamba2)}
\label{sec:exp6}
...
\subsection{Experiment 7: Layer-Wise Collapse Analysis (Mechanistic Interpretability)}
\label{sec:exp7}
...
With 4 tasks tested simultaneously, Bonferroni correction adjusts the significance threshold to $p < 0.0125$. Since all reported $p$-values are $< 10^{-12}$, every comparison survives correction with wide margin.
We note that two-sample $t$-tests assume approximate normality of the test statistic. Given that the underlying accuracy is a binary outcome aggregated to proportions, non-parametric alternatives (e.g., Wilcoxon signed-rank test) may be more appropriate.
...
{\footnotesize Adaptive evaluation results are from independent experimental runs. Non-adaptive values use the balanced 500-sample split (250 adversarial, 250 benign). ``---'' = not computed in this evaluation.}
\end{minipage}
\end{table*}
...
\subsection{Experiment 8: Real-World Deployment Simulation}
\label{sec:exp8}
...
Lexical-stealth constrained attackers face an additional Pareto frontier. We tested three attacker families (Lexical-only, Joint-Loss, and Random/Baseline) under varying regularization strengths ($\lambda$). Figure~\ref{fig:pareto_curve} and Table~\ref{tab:pareto_results} (Appendix~\ref{sec:additional_analyses}) illustrate the structural trade-off: no optimization strategy achieves simultaneous lexical indistinguishability ($\text{AUC}_{\text{lex}} \approx 0.50$) and measurable spectral damage ($\Delta\rho_{\text{mean}} > 0.02$). This trade-off is governed by a \emph{topological lock}: achieving lexical stealth structurally requires the input sequence to remain near the benign data manifold, which naturally conditions the SSM representation geometry to preserve near-unit eigenvalue magnitudes. Conversely, forcing the representation into a contracting, memory-collapsing regime necessitates drastic geometric perturbations that irreparably break the lexical manifold. Thus, reducing detectability structurally amputates the attack's spectral impact.

\begin{figure*}[!t]
\centering
% --- Chart ---
\includegraphics[width=0.55\textwidth]{figures/pareto_curve.png}
\caption{Pareto Frontier of Lexical Evasion ($\text{AUC}_{\text{lex}}$) vs.\ Spectral Damage ($\Delta\rho_{\text{mean}}$). No attack family achieves simultaneous high stealth and significant state disruption. Near-perfect stealth ($\text{AUC}_{\text{lex}} \le 0.60$) caps damage below $\Delta\rho = 0.02$.}
\label{fig:pareto_curve}
\vspace{6pt}
% --- Data table ---
\captionof{table}{Pareto Frontier Results (Mamba-1.4B). Lexical Evasion ($\text{AUC}_{\text{lex}}$) vs.\ Spectral Damage ($\Delta\rho_{\text{mean}}$) across three attacker families and regularization weights $\lambda$.}
\label{tab:pareto_results}
\vspace{3pt}
\begin{tabular}{@{} l c c c @{}}
\toprule
\textbf{Attack Family} & $\bm{\lambda}$ & $\max\,\Delta\rho_{\text{mean}}$ & $\text{AUC}_{\text{lex}}$ \\
\midrule
Lexical-only & 0.00 & 0.0341 & 0.9453 \\
Lexical-only & 0.25 & 0.0262 & 0.9442 \\
Lexical-only & 0.50 & 0.0263 & 0.9444 \\
Lexical-only & 0.75 & 0.0234 & 0.9487 \\
Lexical-only & 1.00 & 0.0368 & 0.9483 \\
\midrule
Joint-Loss   & 0.00 & 0.0808 & 0.9073 \\
Joint-Loss   & 0.25 & 0.1947 & 0.8734 \\
Joint-Loss   & 0.50 & 0.3108 & 0.8253 \\
Joint-Loss   & 0.75 & 0.4521 & 0.7558 \\
Joint-Loss   & 1.00 & 0.5571 & 0.7014 \\
\midrule
Random/Baseline & 0.00 & 0.0173 & 0.5791 \\
Random/Baseline & 0.25 & 0.0178 & 0.5042 \\
Random/Baseline & 0.50 & 0.0185 & 0.5034 \\
Random/Baseline & 0.75 & 0.0132 & 0.5544 \\
Random/Baseline & 1.00 & 0.0139 & 0.5379 \\
\bottomrule
\end{tabular}
\end{figure*}

\paragraph{Quantifying the stealth-damage budget.}
Table~\ref{tab:pareto_results} reveals a sharp asymmetry: the Joint-Loss family achieves $\Delta\rho_{\text{mean}} = 0.557$ at $\lambda=1.0$ but only by surrendering lexical invisibility to $\text{AUC}_{\text{lex}} = 0.701$, while the Lexical-only family sustains $\text{AUC}_{\text{lex}} > 0.94$ at the cost of negligible damage ($\Delta\rho_{\text{mean}} \le 0.037$). Random/Baseline probes cluster in the lower-left quadrant, confirming that significant spectral harm requires deliberate, structure-aware gradient steering---not merely unconstrained token selection. These results formalize a \emph{stealth-damage budget}: each unit of spectral impact an attacker acquires trades compounding surface detectability, with the frontier governed by the Lipschitz constant of the input-to-eigenvalue map (Theorem~\ref{thm:lipschitz_rho}).
...
\begin{equation}
\|\bar{A}^t h_0\|_2 \le \kappa(\bar{A})\, \rho(\bar{A})^t \|h_0\|_2,
\end{equation}
where $\kappa(\bar{A}) = \|V\|_2 \|V^{-1}\|_2$ and $\|V\|_2 \|\Lambda^t\|_2 \|V^{-1}\|_2$ is collapsed via $\|\Lambda^t\|_2 = \rho(\bar{A})^t$.
...
\midrule
Exp 1 (Horizon) & NVIDIA T4 (16GB) & Cloud GPU, $N{=}500$ \\
